            ...
            if (files && files.length > 0) {
                ...
                files.forEach((doc, index) => {
                    ...
                    listItem.innerHTML = `
                        ...
                        <button onclick="showSendDocumentModal(${index})">Отправить документ</button>
                    `;
...
function showSendDocumentModal(index) {
 ...
 const receiverIdInput = document.getElementById('receiverIdInput');
  ...
  sendButton.onclick = function() {
    const receiverId = receiverIdInput.value;
     if (receiverId) {
       sendDocument(index, receiverId);
       modal.style.display = 'none';
     } else {
          alert("Получатель не указан.");
     }
 }
    cancelSendButton.onclick = function() {
        modal.style.display = 'none';
    }
    modal.style.display = 'flex';
}

function sendDocument(index, receiverId) {
  const documentName =  documents[index].file_name;
...
// получение списка полученных документов
async function fetchReceivedDocuments() {
    ...
    try {
        ...
        if (data.status === 'success') {
            console.log("fetchReceivedDocuments: Returning documents:", data.files);
            return data.files;
        } else {
...
function signDocument(index, documentId) {
  const signedDocumentId = documentId;
  // Отправка запроса на сервер для подписания документа
...
function ignoreDocument(index, documentId) {
  const ignoredDocumentId = documentId;
  // Отправка запроса на сервер для игнорирования документа
  fetch(`${SERVER_URL}/ignore_document`, {
    ...
    body: JSON.stringify({
      user_id: userProfile.id,
      document_id: ignoredDocumentId,
    }),
  })
    .then(response => response.json())
    .then(data => {
      if (data.status === 'success') {
        alert("Документ был проигнорирован!");
      } else {
...
// Инициализация данных пользователя при запуске страницы
